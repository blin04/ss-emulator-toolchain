LEXER_SRC = misc/lex.yy.c
PARSER_SRC = misc/parser.tab.c
PARSER_HDR = misc/parser.tab.h

ASM_HEADERS = inc/directives.hpp inc/freftab.hpp inc/instruction.hpp inc/objfile.hpp inc/section.hpp inc/symtab.hpp inc/interface.h
ASM_SOURCES = src/asm.cpp src/directives.cpp src/freftab.cpp src/instruction.cpp src/objfile.cpp src/section.cpp src/symtab.cpp src/interface.cpp $(PARSER_SRC) $(LEXER_SRC)
OUTPUT = build/asembler


asembler: $(ASM_SOURCES)
	g++ -I misc -I inc $(ASM_SOURCES) -o $(OUTPUT)

asembler-debug: $(ASM_SOURCES)
	g++ -g -I misc -I inc $(ASM_SOURCES) -o $(OUTPUT)

lexer: $(LEXER_SRC)

$(LEXER_SRC): misc/lexer.l
	flex -o $@ $<

parser: $(PARSER_SRC) $(PARSER_HDR)
 
$(PARSER_SRC) $(PARSER_HDR): misc/parser.y
	bison -d -o $(PARSER_SRC) $<

clean:
	rm -f $(LEXER_SRC) $(PARSER_SRC) $(PARSER_HDR) $(OUTPUT)